...
{
    public class SoundNlpTest : INlp
    {
        //委托
        ...
        private static int remotePort = 9002;
        ...
        /// 
        /// 
        public SoundNlp(string strIp, int iPort)
        {
            remoteHost = strIp;
            remotePort = iPort;
        }
        ...
        public string SendMsg(string msg)
        {
            if (checkSocketState())
            {
                //Sound 12345678_Ying;5;513;5omT5byA56qX5biY$/r$
                string sendstr = "12345678_Ying" + ";5;513;5omT5byA56qX5biY" + "$/r$" + "\r\n";
                //Console.WriteLine($"注册：{registMsg}");
                byte[] registBuffer = Encoding.UTF8.GetBytes(sendstr);
                theSocket.Send(registBuffer);
                int registLength = theSocket.Receive(result);
